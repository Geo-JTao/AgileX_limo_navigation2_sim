...
class AutoNavigator(Node):
    def __init__(self,node_name):
        ...
        # self.initial_pose = self.set_initial_pose(x=0.0, y=0.0, theta=0.0)
        # 等待Nav2系统完全激活
    ...
    # 手动创建直线路径（仅包含起点和终点）
    def create_straight_path(self,start_pose, goal_pose):
        """创建仅包含起点和终点的直线路径"""
        straight_path = Path()
        straight_path.header.frame_id = start_pose.header.frame_id
        straight_path.header.stamp = self.get_clock().now().to_msg()
        # 路径仅包含起点和终点（两点确定一条直线）
        straight_path.poses.append(deepcopy(start_pose))
        straight_path.poses.append(deepcopy(goal_pose))
        return straight_path

    def plan_and_follow_path(self, start_pose, goal_pose, use_straight=False, smooth=True):
        """
        规划路径并跟踪
        参数:
            start_pose: 起点位姿 (PoseStamped)
            goal_pose: 目标点位姿 (PoseStamped)
            use_straight: 是否使用手动直线路径
            smooth: 是否对规划的路径进行平滑处理
        """
        # 发布起点和终点标记
        self.publish_waypoint_markers([start_pose, goal_pose])
        
        # 根据参数选择路径生成方式
        if use_straight:
            self.get_logger().info("使用手动直线路径")
            path = self.create_straight_path(start_pose, goal_pose)
        else:
            self.get_logger().info("规划路径中...")
            path = self.navigator.getPath(start_pose, goal_pose)
            if not path.poses:
                self.get_logger().error("路径规划失败，未生成任何路径点")
                return False
                
            # 路径平滑处理
            if smooth:
                self.get_logger().info("平滑路径中...")
                path = self.navigator.smoothPath(path)
                if not path.poses:
                    self.get_logger().error("路径平滑失败")
                    return False

        # 发布路径用于RViz可视化
        self.path_pub.publish(path)
        self.get_logger().info(f"路径生成完成，包含 {len(path.poses)} 个点")

        # 跟踪路径
        self.get_logger().info("开始跟踪路径...")
        follow_task = self.navigator.followPath(path)
        ...
        while not self.navigator.isTaskComplete():
            ...
            if feedback:
                self.get_logger().info(
                    f"路径跟踪中 - 剩余距离: {feedback.distance_to_goal:.2f}米",
                    throttle_duration_sec=1.0
                )
        ...
        if result == TaskResult.SUCCEEDED:
            ...
            self.get_logger().error(f"路径跟踪失败，结果代码: {result}")
            return False
...
def main(args=None):
    ...
    try:
        ...
        # 示例1: 使用路径规划和平滑功能从起点到目标点
        start_pose = navigator.set_initial_pose(x=0.0, y=0.0, theta=0.0)
        goal_pose = navigator._create_pose_stamped(x=1.0, y=0.0, theta=0.0)
        navigator.plan_and_follow_path(start_pose, goal_pose, use_straight=False, smooth=True)
        goal_pose1 = navigator._create_pose_stamped(x=2.0, y=0.0, theta=0)
        navigator.plan_and_follow_path(goal_pose, goal_pose1, use_straight=False, smooth=True)
        
        # 示例2: 使用手动直线路径（取消注释启用）
        # goal_pose2 = navigator._create_pose_stamped(x=3.0, y=-2.0, theta=np.pi)
        # navigator.plan_and_follow_path(goal_pose, goal_pose2, use_straight=True, smooth=False)
